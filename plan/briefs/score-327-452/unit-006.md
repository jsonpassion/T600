...
`content/voca/score-327-452/unit-006.md`에 쓴다. 표제어를 바꾸거나 빼거나 더하지 않는다 — 다른 권과의 중복은 이미 제거되어 있다.

## 이 권
- band_id `score-327-452` (327-452, 중급) · unit 006 · 테마: 사회와 복지 — 고용·인구·불평등
- 밴드 성격: 2급·2+급(국가공인 진입). 비즈니스·사회 전반의 빈출 어휘, 기본 콜로케이션과 구동사. 서울대 대학원 최소 기준 327점을 넘기는 구간.

## 파일 형식 — 한 글자도 다르지 않게
```
---
id: voca-327-452-u006
type: voca
level: 2
...
저장 후 `python3 tools/validate_content.py --unit score-327-452/006` 를 실행해 0 errors가 될 때까지 해당 줄만 고친다.
