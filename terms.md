...
육공공(이하 "앱")은 텝스(TEPS) 시험 대비 영어 어휘 학습을 돕기 위해 독립적으로 개발된 iOS 애플리케이션입니다. 앱은 서울대학교 TEPS관리위원회와 무관합니다. 앱은 레벨(점수대)별 학습 콘텐츠를 제공하며, 레벨은 ~326, 327-452, 453-525, 526-600의 네 점수대(New TEPS 공인 등급 기준)입니다. 각 레벨의 단어는 100단어 단위의 책(권)으로 묶여 있고, 레벨마다 1권은 무료로, 나머지 권은 앱 내 구매(이용권)를 통해 이용할 수 있습니다.
...
- **유료 범위:** 2권부터 이용하려면 이용권이 필요합니다. 이용권은 **연간 구독**, **월간 구독**, **평생 이용권** 중 하나이며, 어느 것이든 4개 레벨의 모든 권이 전부 열립니다. 무료 체험 기간은 제공하지 않습니다.
...
- 판매 가격은 App Store에 표시된 금액을 기준으로 하며, 국가와 환율, 프로모션에 따라 다를 수 있습니다.
...
- 학습 진도, 회독 수, 단어 암기 상태 등의 학습 기록은 사용자의 기기 및 사용자의 iCloud 계정(키-값 저장소)에 저장됩니다.
- 개발자는 사용자의 학습 데이터를 별도의 서버에 수집하거나 저장하지 않습니다.
- 앱 내 "학습 데이터 초기화"를 실행하면 기기와 iCloud의 학습 기록이 모두 삭제되며, 이 작업은 되돌릴 수 없습니다.

### 4. 콘텐츠

- 앱의 학습 콘텐츠는 네트워크를 통해 다운로드되며 기기에 캐시됩니다.
- 콘텐츠는 사전 고지 없이 추가되거나 수정될 수 있습니다.
- TEPS는 서울대학교 TEPS관리위원회의 등록 상표입니다. 본 앱은 "시험 대비"라는 서비스의 목적을 설명하기 위한 서술적 용도로만 상표를 언급하며, 서울대학교 TEPS관리위원회와 무관하고 서울대학교 TEPS관리위원회의 제휴나 보증, 승인을 받지 않았습니다.
- 모든 학습 콘텐츠는 자체 제작이며, 실제 시험의 기출문제를 포함하지 않습니다.

### 5. 면책

- 앱은 학습 보조 도구이며, 특정 시험 점수의 달성을 보장하지 않습니다.
- 네트워크 장애 등으로 인한 일시적인 콘텐츠 이용 제한에 대해 개발자는 책임을 지지 않습니다. 단, 이미 다운로드된 콘텐츠는 오프라인에서도 이용할 수 있습니다.
...
Six Hundred ("the App") is an independently developed iOS application that helps users study English vocabulary in preparation for the TEPS test. The App is not affiliated with the Seoul National University TEPS Council. The App provides study content organized by level (score band), and there are four levels based on the New TEPS official grades: up to 326, 327-452, 453-525 and 526-600. Within each level the words are grouped into numbered books of 100 words; the first book of each level is free, and the remaining books can be unlocked through an in-app purchase (a pass).

### 2. Free Tier and In-App Purchases

- **Free tier:** book 1 of the level you choose (10 chapters, 100 words) is free in full. Every feature, including rounds, weak-word review, the 3-second quiz and the level test, works without limits inside the free tier, and no feature is sold separately.
- **Paid tier:** books from the second one onward require a pass: a **yearly subscription**, a **monthly subscription**, or a **lifetime pass**. Each of them unlocks every book across the four levels. No free trial is offered.
...
- Study records such as progress, round counts and word memorization states are stored on your device and in your own iCloud account (key-value store).
- The developer does not collect or store your study data on any separate server.
- Running "Reset Study Data" inside the App deletes the study record from both the device and iCloud. This action cannot be undone.
...
- TEPS is a registered trademark of the TEPS Council of Seoul National University. The mark is used solely in a descriptive (nominative) manner to indicate the purpose of the App, which is test preparation. This App is not affiliated with, endorsed, or approved by 서울대학교 TEPS관리위원회.
- All study content is originally created and does not include actual test questions.

### 5. Disclaimer

- The App is a study aid and does not guarantee any specific test score.
- The developer is not liable for temporary content unavailability caused by network failures. Content already downloaded remains available offline.
